...
for room in rooms:
    ...
    room_name = room[0:room.rindex("-")]
    room_sect = int(room[room.rindex("-")+1:room.index("[")])
    room_csum = room[room.index("[")+1:len(room)-1]
    #print room_name
    #print room_sect
    #print room_csum
    ...
    if len(letters) == 5:
        #print "can be a room, so far"

        letter_counts = []
        for x in room_csum:
            #print x
            letter_counts.append(str(letters[x])+","+x)

        #print letter_counts

        for j in range(4):
            next_letter_count = j + 1
            if letter_counts[j].split(',')[0] >= letter_counts[next_letter_count].split(',')[0]:
                #print "good", j
                #print letter_counts[j].split(',')[0],letter_counts[j].split(',')[1]
                #print "is >= ", letter_counts[next_letter_count].split(',')[0]
                pass
            else:
                #print "numeric order wrong"
                break
                
            if letter_counts[j].split(',')[0] == letter_counts[next_letter_count].split(',')[0]:
                if letter_counts[j].split(',')[1] < letter_counts[next_letter_count].split(',')[1]:
                    #print "good", j
                    #print letter_counts[j].split(',')[1], letter_counts[next_letter_count].split(',')[1]
                    #print "is < ", letter_counts[next_letter_count].split(',')[1]
                    pass
                else:
                    #print "alpha order wrong"
                    break

            if j == 3:
                    sector_id_sum = sector_id_sum + room_sect
                    good_rooms.append(room_name + '-' + str(room_sect))
    else:
        #print "cannot be a room"
        pass


print sector_id_sum
print len(good_rooms)
#print good_rooms

#good_rooms = ["qzmt-zixmtkozy-ivhz-343"]
...
alpha_letters = ["a",
                 ...
                 "y",
                 "z"]
for good_room in good_rooms:
    
    room_name = good_room[0:good_room.rindex("-")]
    room_sect = int(good_room[good_room.rindex("-")+1:])
    #print room_name
    #print room_sect

    decrypted_room_name = ""
    decrypt_shift = room_sect % 26
    #print decrypt_shift
    
    for x in room_name:
        if x != '-':
            letter_index = string_letters.index(x)
            #print letter_index
            #print alpha_letters[letter_index]

            if letter_index + decrypt_shift > 25:
                #print alpha_letters[(letter_index + decrypt_shift) - 26]
                decrypted_room_name = decrypted_room_name + alpha_letters[(letter_index + decrypt_shift) - 26]
            else:
                #print alpha_letters[(letter_index + decrypt_shift)]
                decrypted_room_name = decrypted_room_name + alpha_letters[(letter_index + decrypt_shift)]
        else:
            #print "space"
            decrypted_room_name = decrypted_room_name + " "
            
    print decrypted_room_name, room_sect
