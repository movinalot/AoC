...
ipv7s = ["wysextplwqpvipxdv[srzvtwbfzqtspxnethm]syqbzgtboxxzpwr[kljvjjkjyojzrstfgrw]obdhcczonzvbfby[svotajtpttohxsh]cooktbyumlpxostt",
...
for ipv7 in ipv7s:
    ipv7_segments = re.split('\[|\]',ipv7)
    #print ipv7_segments

    curr_segment = 0
    good_so_far = 0
    
    for ipv7_segment in ipv7_segments:
        ipv7_segment_len = len(ipv7_segment)

        x = 0
        while x < ipv7_segment_len-3:
            if (ipv7_segment[x+0] == ipv7_segment[x+3] and \
               ipv7_segment[x+1] == ipv7_segment[x+2]) and \
               ipv7_segment[x+0] != ipv7_segment[x+1]:

                #print ipv7_segment[x:x+4]
                if curr_segment % 2 == 0:
                    #print "this is an ABBA"
                    good_so_far += 1
                else:
                    #print "this is a hypernet"
                    good_so_far = -1
                    break
...
for ipv7 in ipv7s:
    ...
    for ipv7_segment in ipv7_segments:
        ...
        x = 0
        while x < ipv7_segment_len-2:
            if ipv7_segment[x+0] == ipv7_segment[x+2]:

                #print ipv7_segment[x:x+3]
                if curr_segment % 2 == 0:
                    #print "this is an ABA"
                    abas.append(ipv7_segment[x:x+3])
                else:
                    #print "this is an BAB"
                    babs.append(ipv7_segment[x:x+3])
